...
- **Hallazgos de M5 que siguen abiertos:** `splitIntoMeals` no tiene suelo en 0 aunque hoy ninguna entrada real lo alcanza; `suggestProgression` y `refeedMacros` lanzan con objetos corruptos que el almacén nunca produce. _(Los otros dos de esta entrada están CERRADOS: el atributo sin comillas y los esquemas de URL, en el ataque adversarial a M7; la fuga de URLs de fotos, en M7-1 con el testigo de dibujado.)_
...
- ~~**Recalibrar tira el músculo proyectado también en los perfiles SIN báscula (visto en E11).**~~ **HECHO en V2-M9**, con la opción que el usuario eligió el 8 ago 2026: se arregla para TODOS los perfiles. `src/ui/recalibrate.js:145-178` conserva `otherLeanKg` también en los `estimated` y carga la diferencia a la grasa, con la contrapartida aceptada escrita en el propio código (cambia la duración de los planes ya creados al recalibrarlos). _(Seguía escrito como «no implementado aún»; corregido en E15-7.)_
- **Los hitos de Proyección: dos mecanismos de hito en una vista.** Proyección dibuja los del motor como puntos sobre la línea, con tres contratos de test posicionales colgando; Analizar los dibuja en un carril (`src/ui/marks.js`), y por eso Proyección **no enseña los hitos estéticos ni los de salud** aunque `src/core/health-milestones.js` los calcula con sus fuentes. Anotado con detalle en `docs/v2/PLAN-V2.md`; es la etapa E15-17.
- **Un test E2E intermitente:** `analysis.spec.js` «pulsar un marcador abre su ficha» falla ~1 de cada 4 ejecuciones **solo con `--workers=1`**; pasa en aislamiento y en paralelo, que es como corre CI. Anterior a E15.
- i18n a más idiomas · modo claro · exportación PDF del plan · integración con básculas/wearables · comparativas entre perfiles
- ~~sincronización entre dispositivos~~ **ya no es backlog: es M8/M9**, decidido el 21 ago 2026 (sincronía opt-in, passkeys, cifrado extremo a extremo, D1 + R2). Ver `docs/v2/PLAN-V2.md`.
